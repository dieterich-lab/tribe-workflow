...
library(biomaRt);
mart=useMart(biomart="ENSEMBL_MART_ENSEMBL", dataset="dmelanogaster_gene_ensembl", host="www.ensembl.org")

resMArt=getBM(attributes=c("ensembl_gene_id","external_gene_name","description"),mart=mart)
...
jacusaCond=data.frame(ID=names(sites),sites,SumScore=as.numeric(SumScore));
