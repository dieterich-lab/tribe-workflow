...
list.of.packages <- c("biomaRt", "openxlsx")
...
write.table(unique(data.frame(jacusa.site[,c("Chr","Start","End")],ifelse(jacusa.site[,"Strand"]=="+","A/G","T/C"),"1","+")),file= paste0(gsub(".txt","",args[2]),".bed"),quote=F,sep="\t",append=F, row.names=F, col.names=F)
